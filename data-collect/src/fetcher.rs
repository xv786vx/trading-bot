...
impl Fetcher {
    ...
    pub fn merge_csvs(&self, timeframe: &str) {
        print!("Merging data...");
        stdout().flush().expect("Failed to flush stdout");
        ...
        if timeframe == "1day" {
                    ...
                    for result in reader.records() {
                        ...
                        if let Some(datetime) = record.get(0) {
                            if let Ok(parsed_datetime) =
                                NaiveDate::parse_from_str(datetime, format_string)
                            {
            ...
            let mut sorted_data: Vec<NaiveDate> = datetime_data_map.keys().cloned().collect();
            sorted_data.sort();
